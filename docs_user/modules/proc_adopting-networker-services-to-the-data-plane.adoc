...
= Adopting Networker and Controller services to the {rhos_acro} data plane
...
Adopt each Controller node in your existing {rhos_prev_long} deployment to the {rhos_long} when your Controller node is set as an OVN chassis gateway. Any Controller node with
parameter set to `enable-chassis-as-gw` is considered OVN gateway chassis. In this case, each Controller node will become a edpm networker node after adoption.

FIXME - See if there is a need for a command example on how to check if a Controller has `enable-chassis-as-gw` set?

.Prerequisites

* Define the shell variable. The following value is an example
from a multi node {OpenStackPreviousInstaller} deployment:
+
[subs=+quotes]
----
declare -A networkers
networkers+=(
  ["networker-0.localdomain"]="192.168.122.100"
  ["networker-1.localdomain"]="192.168.122.101"
  ["controller-0.localdomain"]="192.168.122.102"
  # ...
)
----
+
** Replace `["<node-name>"]="192.168.122.100"` with the name and IP address of the corresponding Networker or Controller node.
...
+
[NOTE]
You can reuse most of the `nodeTemplate` section from the `OpenStackDataPlaneNodeSet` CR that is designated for your Compute nodes. You can omit some of the variables because of the limited set of services that are running on Networker/Controllers nodes.
+
[source,yaml]
----
$ oc apply -f - <<EOF
...
spec:
  ...
  nodes:
    networker-0:
      ...
      ansible:
        ansibleHost: ${networkers[networker-0.localdomain]}
      networks:
      - defaultRoute: true
        fixedIP: ${networkers[networker-0.localdomain]}
        name: ctlplane
        subnetName: subnet1
      - name: internalapi
        subnetName: subnet1
    ...
    networker-1:
      ...
      ansible:
        ansibleHost: ${networkers[networker-1.localdomain]}
      networks:
      - defaultRoute: true
        fixedIP: ${networkers[networker-1.localdomain]}
        name: ctlplane
        subnetName: subnet1
      - name: internalapi
        subnetName: subnet1
